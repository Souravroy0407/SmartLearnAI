...
const StudentExamList = () => {
    ...
    const handleGoogleFormClick = async (exam: Exam) => {
        // Just open the link. We might want to mark as submitted automatically or ask user to confirm?
        // For now just open it.
        window.open(exam.external_link, '_blank');
        // Optionally mark as "viewed" or let them manually mark done?
        // The implementation plan said "Mark as Done". I'll add a button for that if needed, 
        // but for now I'll just leave it pending or maybe auto-submit an empty "done" payload?
        // Let's keep it simple: They open link. They can click "Mark as Done" separately.
    };
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {examStatuses.map(({ exam, status, marks }) => (
                    <div key={exam.id} className="bg-white p-6 rounded-2xl shadow-sm border border-secondary-light/20 flex flex-col h-full">
